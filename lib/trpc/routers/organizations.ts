...
export const organizationsRouter = router({
  ...
  getOrganization: protectedProcedure.input(getOrganizationSchema).query(async ({ input }) => {
    ...
    // Fetch custom fields from database (Better Auth doesn't return these)
    ...
    // Merge Better Auth data with custom fields
    return {
      ...result,
      companyDescription: orgCustomFields?.companyDescription ?? null,
      productDescription: orgCustomFields?.productDescription ?? null,
      serviceDescription: orgCustomFields?.serviceDescription ?? null,
    };
  }),
  ...
  /**
   * Update organization company information (descriptions)
   */
  updateCompanyInfo: protectedProcedure
    .input(updateOrgCompanyInfoSchema)
    .mutation(async ({ input }) => {
      const organization = await auth.api.getFullOrganization({
        query: {
          organizationId: input.organizationId,
      ...
      // Update organization descriptions directly in database
      const updateData: {
      ...
      if (input.companyDescription !== undefined) {
        updateData.companyDescription = input.companyDescription || null;
      }
      if (input.productDescription !== undefined) {
        updateData.productDescription = input.productDescription || null;
      }
      if (input.serviceDescription !== undefined) {
        updateData.serviceDescription = input.serviceDescription || null;
      }
